...
function createHierarchyCard(username, userData, rank) {
  ...
  if (!isCurrentUser) {
    ...
    // FIXED: Gift button with better error handling and fallback
    ...
    giftBtn.onclick = (e) => {
      ...
      console.log('🎁 Gift button clicked for user:', username);
      console.log('🎁 Checking for openGiftModal function...');
      console.log('🎁 window.openGiftModal exists:', typeof window.openGiftModal);
      console.log('🎁 openGiftModal exists:', typeof openGiftModal);
      
      // Try multiple ways to access the gift modal function
      if (typeof window.openGiftModal === 'function') {
        console.log('✅ Calling window.openGiftModal');
        window.openGiftModal(username);
      } else if (typeof openGiftModal === 'function') {
        console.log('✅ Calling openGiftModal');
        openGiftModal(username);
      } else {
        // Fallback: show error with instructions
        console.error('❌ Gift modal function not found. Please ensure chat.js is loaded.');
        console.log('Available window functions:', Object.keys(window).filter(k => k.includes('Gift')));
        showVotingError('Gift system is still loading. Please wait a moment and try again.');
        
        // Try to reload chat system
        setTimeout(() => {
          if (typeof initChat === 'function') {
            console.log('Attempting to reinitialize chat system...');
            initChat();
          }
        }, 500);
      }
