/**
 * js/chat.js - Chat + Gift handling (robust) (modified)
 *
 * Fixes / improvements:
 * - Keep localStorage key 'pyramidUser' everywhere.
 * - Prevent HTML injection in regular messages (use textContent).
 * - Safer Audio creation (wrapped in try/catch).
 * - Some defensive checks and error logging.
 * - Add a star (⭐) next to a user's name in chat if they have voted this week
 *   (determined by votesRemaining < 3 in the users collection).
 */
...
const sounds = (() => {
  const s = {};
  try { s.normal = new Audio('https://assets.mixkit.co/active_storage/sfx/2869/2869-preview.mp3'); } catch(e){ s.normal = { play: ()=>{} }; console.warn('Audio init failed:', e); }
  try { s.cool = new Audio('https://assets.mixkit.co/active_storage/sfx/1435/1435-preview.mp3'); } catch(e){ s.cool = { play: ()=>{} }; }
  try { s.legendary = new Audio('https://assets.mixkit.co/active_storage/sfx/2019/2019-preview.mp3'); } catch(e){ s.legendary = { play: ()=>{} }; }
  try { s.system = new Audio('https://assets.mixkit.co/active_storage/sfx/951/951-preview.mp3'); } catch(e){ s.system = { play: ()=>{} }; }
  return s;
})();
...
function initChat() {
  ...
  if (chatInput) {
    ...
    chatInput.addEventListener('blur', () => {
      if (!myName) return;
      db.collection("status").doc(myName).set({ typing: false, time: Date.now() }).catch(err => {
...
function renderMessage(m, container) {
  ...
  if (isGift) {
    ...
    // Add a star badge if this user has voted this week (votesRemaining < 3)
    if (typeof db !== 'undefined' && m.user) {
      db.collection('users').doc(m.user).get().then(doc => {
        try {
          const votesLeft = (doc.exists && doc.data() && typeof doc.data().votesRemaining !== 'undefined') ? doc.data().votesRemaining : 3;
          if (votesLeft < 3) {
          ...
          console.error('Error processing votesRemaining for user star:', e);
        }
      }).catch(err => {
        // Non-fatal: if the user doc can't be fetched, just skip the star
        console.debug('Failed to fetch user votes for star:', err);
      });
    }
...
// If a global openGiftMenu was defined earlier in some other script, prefer that.
