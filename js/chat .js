...
function createTextMessageElement(message) {
  ...
  // Gift button (if not own message and user is logged in)
  if (ChatState.currentUser && message.user !== ChatState.currentUser && message.user !== 'SYSTEM') {
    const giftBtn = document.createElement('button');
    giftBtn.className = 'text-[10px] px-3 py-1 bg-zinc-800/50 border border-zinc-700 rounded hover:border-[#d4af37] hover:bg-[#d4af37]/10 transition-all flex items-center gap-1';
    giftBtn.innerHTML = '<span>🎁</span><span>Gift</span>';
    giftBtn.onclick = (e) => {
      e.preventDefault();
      e.stopPropagation();
      openGiftModal(message.user);
    };
